...
pub fn gstreamer_stream() -> impl Stream<Item = Result<VideoFrame, VideoError>> {
    iced::stream::channel(1, |mut tx: Sender<Result<VideoFrame, VideoError>>| async move {
        ...
        let (gst_tx, mut gst_rx) = watch::channel::<Option<Result<VideoFrame, VideoError>>>(None);
