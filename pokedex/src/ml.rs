...
use ort::{execution_providers::CoreMLExecutionProvider, inputs};
...
fn image_to_tensor(img: &DynamicImage) -> Array4<f32> {
    ...
    let mut tensor: ndarray::ArrayBase<ndarray::OwnedRepr<f32>, ndarray::Dim<[usize; 4]>, f32> =
...
) -> Result<(usize, f32), anyhow::Error> {
    let buff: image::ImageBuffer<image::Rgba<u8>, Vec<u8>> =
        image::ImageBuffer::from_vec(frame.width, frame.height, frame.data.clone())
